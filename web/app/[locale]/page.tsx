...
export default async function Home({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      ...
      <div className="frame">
        <EphemerisRail locale={locale} eyebrow={dict.rail.eyebrow} note={dict.rail.note} />

        <main className="main">
          <section className="hero">
            <SkyWheel alt={dict.hero.wheelAlt} />
            ...
            <table className="packsTable">
              ...
              <tbody>
                {dict.credits.packs.map((pack) => (
                  <tr key={pack.credits} className={pack.popular ? "packPopular" : undefined}>
                    <th scope="row">
                      {pack.credits}
                      {pack.popular ? <span className="tag">{dict.credits.popular}</span> : null}
                    </th>
                    <td>{pack.price}</td>
              ...
              <tbody>
                {dict.credits.terms.map((term) => (
                  <tr key={term.label}>
                    ...
                    <td className={term.free ? "free" : undefined}>{term.value}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="stores">
              ...
              <a className="store" href="#">
                <svg width="19" height="21" viewBox="0 0 19 21" aria-hidden="true">
                  <path fill="currentColor" d="M1.1 1.2C.9 1.5.8 1.9.8 2.5v16c0 .6.1 1 .3 1.3l8.4-8.8L1.1 1.2z" />
                  <path fill="currentColor" d="M10.6 10.1l2.7-2.8L2.9.6C2.4.3 1.9.2 1.5.4l9.1 9.7z" />
                  <path fill="currentColor" d="M10.6 11.9L1.5 20.6c.4.2.9.1 1.4-.2l10.4-6.7-2.7-1.8z" />
                  <path fill="currentColor" d="M17.4 9.2l-2.9-1.9-3 3.2 3 3.1 2.9-1.9c.9-.6.9-1.9 0-2.5z" />
                </svg>
                <span className="storeText">
